...
- [ ] **주문서 작성 (`Checkout.vue`)**: 배송지 선택, 거래 방식(택배/직거래) 토글, 결제 금액 명세서.
- [ ] **토스페이먼츠(Toss Payments) 연동**: SDK 연동, `requestPayment` 시 `useEscrow: true` 파라미터 적용.
- [ ] **결제 결과 처리**: 결제 성공(`OrderSuccess.vue`) 및 실패(`OrderFailure.vue`) 전용 안내 페이지 및 백엔드 승인 API 호출 연동.
- [ ] **실시간 상태 업데이트**: SSE 알림 연동 및 결제 상태(에스크로 배송 완료 등) UI 반영.
