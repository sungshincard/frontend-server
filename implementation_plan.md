...
- `PokemonGrid.vue`: 메인 포켓몬 도감 바둑판 배열 (세대별 필터 및 포켓몬 전체 이미지 표출) (완료, 현재 `Dex.vue`)
- 포켓몬 클릭 시 연동된 `CardCatalog.vue`로 이동하여 해당 포켓몬의 판매 중인 카드 리스트 표출 (완료, 현재 `Cards.vue`)
- [MOD] `Cards.vue`: 하드코딩된 필터 옵션(카테고리, 타입 등)을 백엔드 메타데이터 API와 연동
